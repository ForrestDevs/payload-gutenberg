...
import { traverseRichText } from "./traverse-richtext";
            ...
            result[fieldName] = incomingData[fieldName].map(
              (incomingBlock, i) => {
                ...
                const incomingBlockJSON =
                  fieldSchema.blocks[incomingBlock.blockType];

                console.log(incomingBlockJSON);
          ...
          if (fieldSchema.hasMany && Array.isArray(incomingData[fieldName])) {
            ...
            incomingData[fieldName].forEach((incomingRelation, i) => {
              ...
              if (Array.isArray(fieldSchema.relationTo)) {
                ...
                const hasChanged =
                  newID !== oldID || newRelation !== oldRelation;
                ...
                const hasChanged =
                  incomingRelation !== result[fieldName][i]?.id;

                const hasUpdated =
                  fieldSchema.relationTo ===
                    externallyUpdatedRelationship?.entitySlug &&
