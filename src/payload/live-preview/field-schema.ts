// @ts-nocheck
import type { FieldTypes, ClientField, ClientConfig } from "payload";
    ...
    switch (field.type) {
      ...
      case "blocks":
        acc.push({
          name: field.name,
          type: field.type,
          blocks: (field.blockReferences ?? field.blocks).reduce(
            (acc, _block) => {
              const block =
                typeof _block === "string" ? config.blocksMap[_block] : _block;
              acc[block.slug] = {
                fields: fieldSchemaToJSON(
                  [
                    ...block.fields,
