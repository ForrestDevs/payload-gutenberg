...
import type { LivePreviewMessageEvent } from "./types";
import { isLivePreviewEvent } from "./is-live-preview";
...
export const handleMessage = async <T>(args: {
  ...
  if (isLivePreviewEvent(event, serverURL)) {
    const { data, externallyUpdatedRelationship, fieldSchemaJSON, locale } =
      event.data;
